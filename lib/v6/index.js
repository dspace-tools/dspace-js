...
const expantPath = (path, params = {}) => {
  if (Array.isArray(params)) {
    ...
    return params.reduce((prev, c) =>  prev.replace(/{\w+}/, c), path)
    
  } else {
    return Object.keys(params).reduce((prev, c) => prev.replace(`{${c}}`, params[c]),  path)
  }
}
...
module.exports = ({ hostname, ssl = true }) => {
  ...
  Object.keys(v6Schema).forEach(methodName => {
    ...
    const { method, path, transform, inputFormat, pagination } = v6Schema[methodName]

    console.log("building", methodName )

    client[methodName] = (params = {}, ...pathParams) => {
      console.log(pathParams)
      const { sessionId } = params
      
      const uri = uriWithQuery(buildUri(expantPath(path, pathParams)), params)
